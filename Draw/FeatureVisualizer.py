...
class FeatureVisualizer:
    def __init__(self, filtered_data,features,*xml):
    ...
    def SingleFeaturesVisualizer(self):
        data=self.filtered_data
        # 建一个PDF文件
        now = datetime.now().strftime("%Y%m%d_%H%M")
        pdf_filename = f'graph/1/single_features_plots{now}.pdf'
    ...
    def SelectedFeatureVisualizer(self):
        # 建一个PDF文件
        now = datetime.now().strftime("%Y%m%d_%H%M")
        pdf_filename = f'graph/1/Selected_features_plots{now}.pdf'
        pdf_pages = PdfPages(pdf_filename)
        data = self.filtered_data
        # 使用combinations生成所有不重複的3個欄位名稱的組合
        combinations_list = list(combinations(self.features, 3))

...
class GetFilterData:
    ...
    def getFliterData(self):
        ...
        computed_mkdata['最高價當日-指數變化']=(computed_mkdata['最高價當日-指數收盤價(元)'] - computed_mkdata['指數收盤價(元)']).round(2)
        computed_mkdata['最高價當日-指數變化率']=((computed_mkdata['最高價當日-指數收盤價(元)'] - computed_mkdata['指數收盤價(元)'])/computed_mkdata['指數收盤價(元)']*100).round(2)
        #整合價量、公司資料、大盤
        flited_data_sm=pd.merge(max_price_mkdata,computed_mkdata[['證券代碼','最高價當日-指數變化','最高價當日-指數變化率']],on='證券代碼',how='left')
        #取得最高價當日籌碼資訊
        ...
        computed_chdata['最高價當日-董監持股變化']=(computed_chdata['最高價當日-董監持股數'] - computed_chdata['董監持股數']).round(2)
        computed_chdata['最高價當日-融資餘額(張)變化']=(computed_chdata['最高價當日-融資餘額(張)'] - computed_chdata['融資餘額(張)']).round(2)
        computed_chdata['最高價當日-融券餘額(張)變化']=(computed_chdata['最高價當日-融券餘額(張)'] - computed_chdata['融券餘額(張)']).round(2)
        computed_chdata['最高價當日-200-400 張(比率)變化']=(computed_chdata['最高價當日-200-400 張(比率)'] - computed_chdata['200-400 張(比率)']).round(2)
        computed_chdata['最高價當日-400-600 張(比率)變化']=(computed_chdata['最高價當日-400-600 張(比率)'] - computed_chdata['400-600 張(比率)']).round(2)
        computed_chdata['最高價當日-600-800 張(比率)變化']=(computed_chdata['最高價當日-600-800 張(比率)'] - computed_chdata['600-800 張(比率)']).round(2)
        computed_chdata['最高價當日-800-1000張(比率)變化']=(computed_chdata['最高價當日-800-1000張(比率)'] - computed_chdata['800-1000張(比率)']).round(2)
        computed_chdata['最高價當日-1000張以上(比率)變化']=(computed_chdata['最高價當日-1000張以上(比率)'] - computed_chdata['1000張以上(比率)']).round(2)


        computed_chdata['最高價當日-董監持股變化率']=((computed_chdata['最高價當日-董監持股數'] - computed_chdata['董監持股數'])/computed_chdata['董監持股數']*100).round(2)
        computed_chdata['最高價當日-融資餘額(張)變化率'] = np.where(
            computed_chdata['融資餘額(張)'] == 0,  # 分母為0的情況
            0,  # 當分母為0時，將結果設為0
            ((computed_chdata['最高價當日-融資餘額(張)'] - computed_chdata['融資餘額(張)']) / computed_chdata['融資餘額(張)'] * 100).round(2)
        )
        computed_chdata['最高價當日-融券餘額(張)變化率'] = np.where(
        ...
        flited_data_smc=pd.merge(max_price_chdata,computed_chdata[['證券代碼','最高價當日-董監持股變化','最高價當日-融資餘額(張)變化','最高價當日-融券餘額(張)變化','最高價當日-200-400 張(比率)變化','最高價當日-400-600 張(比率)變化','最高價當日-600-800 張(比率)變化','最高價當日-800-1000張(比率)變化','最高價當日-1000張以上(比率)變化','最高價當日-董監持股變化率','最高價當日-融資餘額(張)變化率','最高價當日-融券餘額(張)變化率']],on='證券代碼',how='left')
